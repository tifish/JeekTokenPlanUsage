...
实现：[AutoUpdate.cs](../JeekTokenPlanUsage/AutoUpdate.cs)、镜像选择用 [JeekTools.NET](../JeekTools.NET) 的 GitHubMirrors、文本下载用 [MirrorHttp.cs](../JeekTokenPlanUsage/MirrorHttp.cs)、[bin/AutoUpdate.ps1](../bin/AutoUpdate.ps1)；调度方：[TrayApplicationContext.cs](../JeekTokenPlanUsage/TrayApplicationContext.cs)；CI：[.github/workflows/dotnet-desktop.yml](../.github/workflows/dotnet-desktop.yml)。
...
- 运行时反射读 `Assembly.GetExecutingAssembly().GetName().Version.Major`（[AutoUpdate.cs `ReadLocalCommitCount`](../JeekTokenPlanUsage/AutoUpdate.cs)）
...
csproj 默认 `<Version>0</Version>`（[JeekTokenPlanUsage.csproj](../JeekTokenPlanUsage/JeekTokenPlanUsage.csproj)）。本地 `dotnet build` 不带 `/p:Version=...` → `AssemblyVersion=0.0.0.0` → `Major=0`，AutoUpdate 检测到后返回 `Failed("local version unavailable")`，**绝不**会用 release 把开发者未提交的工作树覆盖掉。CI 用 `/p:Version=<count>` 覆盖这个 0。

## 三态结果

[`UpdateCheckOutcome`](../JeekTokenPlanUsage/AutoUpdate.cs) 区分三种结局，避免把网络失败误报成"已是最新版本"：
...
`Failed` 的具体原因写在 [`AutoUpdate.FailureReason`](../JeekTokenPlanUsage/AutoUpdate.cs) 属性，由 `Fail(reason)` 辅助统一设置 + 写 [Log](../JeekTokenPlanUsage/Log.cs)。常见值：
...
- `.gitignore` 把 `bin/` 改成 `bin/*` 让单文件例外生效，再加 `!bin/AutoUpdate.ps1`、`!bin/Setup.cmd`、`!bin/dotnet-install.ps1`（[.gitignore](../.gitignore)）
- `Publish.cmd` 清理 bin 时用 PowerShell `$keep` 列表跳过这些已入库脚本（[Publish.cmd](../Publish.cmd)）
- csproj 不需要 `<None Update>` 拷贝——脚本天然就在 bin 里

### 镜像选择

[JeekTools.NET](../JeekTools.NET) 的 `GitHubMirrors` 内置三个候选：
...
首次调用 `GetFastestMirror` 时三路并行 `GET` 0..100KB Range，最快返回 200/206 的胜出，**索引缓存在静态字段**；后续 zip 下载与 version.txt 下载复用同一索引，第二次调用 O(1)。`ResetFastestMirror()` 可清缓存。探测走裸 `HttpClient`，已通过 `HttpClient.DefaultProxy`（在 `Program.Main` 装上 [AppProxy](../JeekTokenPlanUsage/AppProxy.cs)）遵循应用代理设置。

`DisableMirrorDownload`（[AppSettings.cs](../JeekTokenPlanUsage/AppSettings.cs)）默认关闭，海外用户若觉得镜像反而更慢可打开走直连。
...
[AppSettings.cs](../JeekTokenPlanUsage/AppSettings.cs) 两个相关字段：
...
所有检查与更新动作写到 [Log](../JeekTokenPlanUsage/Log.cs)（基于 JeekTools.NET 的 `LogManager`/ZLogger）：`%LocalAppData%\JeekTokenPlanUsage\Logs\`（按天与大小滚动，保留 7 天）。典型条目：

```
2026-06-22 14:00:42.489 [INF] [App] AutoUpdate check started (manual=False)
2026-06-22 14:00:43.012 [INF] [App] AutoUpdate: local=234 remote=235 — update available
2026-06-22 14:00:43.821 [INF] [App] AutoUpdate: launched updater; exiting
```

失败示例：

```
2026-06-22 14:00:42.489 [INF] [App] AutoUpdate check started (manual=True)
2026-06-22 14:00:47.512 [WRN] [App] AutoUpdate: no reachable mirror
```
