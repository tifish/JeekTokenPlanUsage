...
async Task HandleAsync(JsonNode message)
{
    ...
    // Only a real tool call is worth starting the GUI for: MCP clients open
    // stdio servers when a session begins, and popping the tray app on every
    // session start would be rude.
...
// The app is unreachable. Keep the session usable instead of failing the
// handshake: the client stays connected, and only real tool calls report why
// nothing happened.
JsonNode OfflineResponse(string? method, JsonNode? id, string reason) => method switch
{
    ...
    "tools/call" => AdapterText.RpcResult(id, new JsonObject
    {
        ...
        {
            ["type"] = "text",
            ["text"] = $"JeekTokenPlanUsage is not reachable on {options.DescribePipes()}. "
                       + $"Start the app and retry. Details: {reason}",
        }),
        ["isError"] = true,
    }),
    _ => AdapterText.RpcError(id, -32601, $"Method not available while JeekTokenPlanUsage is closed: {method}"),
};
    ...
    string AppPath)
{
    ...
    public static AdapterOptions Parse(string[] args)
    {
        ...
        // specific name first and fall back to the bare one.
        List<string> pipes;
        if (pipe is { Length: > 0 })
        {
            pipes = [pipe];
        ...
        // open; only the product surface starts it on demand.
...
internal sealed class PipeConnection(AdapterOptions options) : IDisposable
{
    ...
    /// <summary>
    /// Forwards one message and returns the matching response line, or null when
    /// the message was a notification. Retries once on a broken pipe so an app
    /// restart does not end the agent's session.
    /// </summary>
    public async Task<string?> SendAsync(JsonNode message, bool expectsResponse, bool mayLaunch)
    {
        ...
        for (var attempt = 0; ; attempt++)
        {
            try
            {
                ...
                // Skip server-initiated notifications so they cannot be mistaken
                // for the reply to this request (the pipe is duplex).
    ...
    private async Task<(StreamReader Reader, StreamWriter Writer)> ConnectAsync(bool mayLaunch)
    {
        ...
        {
            LaunchApp();
            // The tray app has to start and register the pipe.
    ...
    private void LaunchApp()
    {
        if (!File.Exists(options.AppPath))
            throw new FileNotFoundException("JeekTokenPlanUsage executable not found.", options.AppPath);
